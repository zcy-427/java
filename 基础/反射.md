...
//解析注解  
Method testMethod = MyAnnotation.class.getMethod("test");  
MyAnnotation myAnnotation = testMethod.getAnnotation(MyAnnotation.class);  
System.out.println(myAnnotation.value()); // 测试注解
```

### 3. 动态代理（JDK 动态代理）
基于反射实现 JDK 动态代理（AOP 的核心）：
...
public class Dynamic_Proxy {  
    public static void main(String[] args) {  
        ...
        Service proxyInstance = (Service) Proxy.newProxyInstance(  
                ...
                service.getClass().getInterfaces(),//目标类实现的接口  
                new ProxyHandler(service)//代理处理器  
        );  
        proxyInstance.doService();  
    }  
}
```
...
执行流程（关键链路）
```plaintext
1. 执行proxy.doService() → 代理对象的doService方法被调用
2. JVM拦截此调用，自动触发ProxyHandler的invoke方法
3. 执行invoke方法内的「前置增强」→ 打印“前置增强”
4. 通过反射调用target.doService() → 执行ServiceImpl的doService，打印“执行业务逻辑”
5. 执行invoke方法内的「后置增强」→ 打印“后置增强”
6. invoke方法返回null，整个调用结束
```
