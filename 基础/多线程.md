...
- [x] **避免锁对象被修改**：锁对象若被重新赋值（如 `lock = new Object()`），会导致后续线程使用新锁，失去同步效果。
- [x] **锁对象必须是引用类型**：不能用基本类型（如 `int`、`long`），因为基本类型不是对象，无法关联 Monitor。
- [x] **避免使用 String 常量 / 包装类作为锁**：String 常量池会导致不同地方的字符串对象指向同一实例，可能意外共享锁；包装类（如 `Integer`）的缓存机制也会导致类似问题。
