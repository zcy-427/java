...
### URL编程
*Java 提供了 `java.net` 包下的一系列类实现 URL（统一资源定位符）编程，核心用于访问网络资源（如 HTTP/HTTPS/FTP 等协议的资源）、发送 HTTP 请求、读取网络响应等。*

#### 关键类

|类名|作用|
|---|---|
|`URL`|封装 URL 地址，提供资源访问的入口，是所有 URL 编程的基础|
|`URLConnection`|URL 的连接抽象类，提供细粒度的连接控制（如设置请求头、超时、读写数据）|
|`HttpURLConnection`|`URLConnection` 的子类，专门处理 HTTP/HTTPS 协议，扩展了 HTTP 特有功能（如响应码、请求方法）|

##### URL
`URL` 类代表一个具体的网络资源地址，通过它可以直接打开资源流，或创建 `URLConnection` 进行高级操作。

**构造方法**
`URL` 的构造方法会抛出 `MalformedURLException`（URL 格式非法），需捕获或声明抛出。常用构造方法：

| 构造方法                                                       | 说明                                                                            |
| ---------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `URL(String spec)`                                         | 直接传入完整 URL 字符串（如 `new URL("https://www.baidu.com")`）                          |
| `URL(String protocol, String host, int port, String file)` | 分部分构造（协议、主机、端口、资源路径），如 `new URL("http", "localhost", 8080, "/api/test")`      |
| `URL(URL context, String spec)`                            | 基于上下文 URL 构造相对路径（如上下文是 `http://a.com/`，spec 是 `b.txt` → `http://a.com/b.txt`） |

**核心方法**
`URL` 类提供了获取 URL 组成部分和访问资源的核心方法：

| 方法                 | 返回值           | 说明                                                    |
| ------------------ | ------------- | ----------------------------------------------------- |
| `getProtocol()`    | String        | 获取协议（http/https/ftp 等）                                |
| `getHost()`        | String        | 获取主机名 / IP 地址                                         |
| `getPort()`        | int           | 获取端口号，无指定则返回 -1（使用协议默认端口）                             |
| `getPath()`        | String        | 获取资源路径                                                |
| `getQuery()`       | String        | 获取查询参数（? 后的部分）                                        |
| `getRef()`         | String        | 获取锚点（# 后的部分）                                          |
| `openStream()`     | InputStream   | 打开资源的输入流（简化版，等价于 `openConnection().getInputStream()`） |
| `openConnection()` | URLConnection | 创建 URL 连接对象，用于高级操作（如设置请求头、发送 POST 数据）                 |

##### URLConnection 类：高级连接控制
`URLConnection` 是抽象类，封装了与 URL 资源的连接，支持设置请求头、超时、读写数据等高级操作，是实现 HTTP 请求的核心。

**核心的使用流程**
1. 通过 `URL.openConnection()` 获取 `URLConnection` 实例（或强转为 `HttpURLConnection`）；
2. 设置连接属性（超时、请求头、是否允许输出等）；
3. （可选）调用 `connect()` 主动建立连接（`getInputStream()` 会自动触发连接）；
4. 读取响应（输入流）或发送数据（输出流）；
5. 关闭流资源（推荐用 `try-with-resources` 自动关闭）。

**常用方法**

| 方法                                             | 说明                                                    |
| ---------------------------------------------- | ----------------------------------------------------- |
| `setConnectTimeout(int timeout)`               | 设置连接超时（毫秒），避免程序挂起                                     |
| `setReadTimeout(int timeout)`                  | 设置读取超时（毫秒）                                            |
| `setDoInput(boolean doInput)`                  | 是否允许读取响应（默认 true）                                     |
| `setDoOutput(boolean doOutput)`                | 是否允许发送数据（默认 false，POST 请求需设为 true）                    |
| `setRequestProperty(String key, String value)` | 设置请求头（如 `User-Agent`、`Content-Type`）                  |
| `connect()`                                    | 主动建立连接（非必需）                                           |
| `getInputStream()`                             | 获取响应输入流（读取服务器返回的数据）                                   |
| `getOutputStream()`                            | 获取请求输出流（向服务器发送数据）                                     |
| `getHeaderField(String name)`                  | 获取指定响应头（如 `Content-Type`）                             |
| `getResponseCode()`                            | （HttpURLConnection 特有）获取 HTTP 响应码（200 = 成功，404 = 不存在） |
示例1：读取URL资源
...
public class URL_resource_read {  
    public static void main(String[] args) {  
        try {  
...
public class http_post {  
    public static void main(String[] args) {  
        String urlstr = "https://httpbin.ceshiren.com/post";  
        String charset = "UTF-8";  
        String params = null;  
        try {  
            params = "name=" + URLEncoder.encode("zhangsan", charset)  
                    + "&age=" + URLEncoder.encode("25", charset);  
        ...
        {  
            ...
            {  
                System.out.println("POST请求成功，响应码：" + responseCode);  
...
public class http_get {  
    public static void main(String[] args) {  
        String urlstr = "https://httpbin.ceshiren.com/get?name=张三&age=20";  
        URL url = null;  
        try {  
            url=new URL(urlstr);  
        }catch (MalformedURLException e)  
        ...
        {  
            ...
            connection.setReadTimeout(5000);//设置读取超时时  
            connection.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");  
            connection.setRequestProperty("User-Agent","Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0");//设置请求头  
